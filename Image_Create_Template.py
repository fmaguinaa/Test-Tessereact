...
fileName = f'{directory}/config/data//ADMIN1.jpg'
...
fig = px.imshow(fig)
shapes=pd.read_csv(f'{directory}/config/data/shapeSugarLand1.csv')
row_1=shapes.iloc[0]
# print(row_1)
...
def on_new_annotation(string):
    ...
    if "shapes" in string:
            ...
            for index, row in data.iterrows():
                ...
                # print(text)
                dfReadingSection = pd.DataFrame(dict(shape=index,label=row['label']['text'],text=StringIO(text)))
                ...
                # print(dfReadingSection)
                data2 = pd.concat([data2, dfReadingSection])
